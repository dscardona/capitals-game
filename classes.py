    ...
    def level1(self, score, attempts):
        ...
        while True:
            if question_list != []:
                ...
                if selection == "0":
                    return score
                elif selection == question_capital or (selection in options_menu and options_menu[selection] == question_capital):
                    question_list.remove((question_country, question_capital))
                    score += 1
                    print(f"You've answered correctly! Score: {score} -- Attempts left: {attempts}")
                    continue
                else:
                    attempts -= 1
                    if attempts <= 0:
                        ...
                        print(f"Wrong answer. Remaining attempts: {attempts}. Score: {score}")
            else:
                print(f"You've completed level 1 with a score of {score} points and {attempts} attempt(s) left")
                return score